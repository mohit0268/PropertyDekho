...
const PropertyCard = ({ property }) => {
  const { type, name, rates, beds, baths, square_feet, location } = property;
    ...
    <div class="rounded-xl shadow-md relative">
      ...
      <div class="p-4">
        ...
        <h3 class="absolute top-[10px] right-[10px] bg-white px-4 py-2 rounded-lg text-blue-500 font-bold text-right md:text-center lg:text-right">
          {rates.monthly ? `${rates.monthly}/mo` : `${rates.weekly}/we`}
        </h3>

        <div class="flex justify-center gap-4 text-gray-500 mb-4">
          <p>
            <FaBed className="inline mr-2">{beds}</FaBed>
            <span class=" lg:inline">Beds</span>
          </p>
          <p>
            <FaBath className="inline mr-2">{baths}</FaBath>
